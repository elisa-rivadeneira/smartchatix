...
if ($containsSalesQuery) {
    ...
    $salesInfo = "Aquíii tienes un resumen de las ventas. La primera columna es cliente:\n";
    $salesInfo .= "| Cliente          | Producto                            | Precio unitario | Fecha Venta | Cantidad | Subtotal |\n";
    $salesInfo .= "|------------------|-------------------------------------|-----------------|-------------|----------|----------|\n";

    foreach ($ventas as $venta) {
        $salesInfo .= "| {$venta->cliente_nombre} | {$venta->producto} | {$venta->precio_unitario} | {$venta->fecha_venta} | {$venta->cantidad} | {$venta->total} |\n";
    }
