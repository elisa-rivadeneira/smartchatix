        ...
        <div id="chat-history">
            @foreach($chatHistories as $chat)
                <div class="mb-2">
                    ...
                    <p><strong>Asistente:</strong> {{ $chat->assistant_response }}</p>
                    <hr>
                </div>
            @endforeach
        </div>
...
@push('scripts')
    ...
    <script>
        $(document).ready(function() {
            ...
            console.log("El script de JavaScript se ha cargado correctamenteeeeeeeeeeeeeeeee");

            $('#chat-form').on('submit', function(event) {
                event.preventDefault(); // Evitar el envío tradicional del formulario
                console.log("Formulario enviado vía AJAX"); // Verifica en la consola del navegador
                debugger; // Esto detendrá la ejecución del código en este punto
            ...
            success: function(response) {
                ...
                $('#chat-history').append(
                    '<div class="mb-2">' +
                        ...
                        '<p><strong>Asistente:</strong> ' + response.assistant_response + '</p>' +
                        '<hr>' +
                    '</div>'
                );
            },
